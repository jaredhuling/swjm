...
JFM data: n = `r length(unique(Data_jfm$id))` subjects, `r sum(Data_jfm$event)` readmission events, `r sum(Data_jfm$status)` deaths
...
- alpha matrix: `r nrow(fit_jfm$alpha)` rows x `r ncol(fit_jfm$alpha)` columns
- beta matrix: `r nrow(fit_jfm$beta)` rows x `r ncol(fit_jfm$beta)` columns
